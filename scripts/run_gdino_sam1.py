"""Run Grounded SAM 1 over every preprocessing on a WoodScape split, saving one report per cell.

    uv run python scripts/run_gdino_sam1.py --data-dir data --metrics-dir metrics --cache-dir cache

Loads a single model, so it fits in memory. Re-runnable: finished cells are skipped and
