"""Rectify processor: fisheye image to a central virtual pinhole view and back.
...
from fishi.preprocess.gnomonic import focal
...
def forward_maps(
    ...
    rays = np.stack(
        [
            (columns - center_x) / focal_pixels,
            (rows - center_y) / focal_pixels,
...
class Rectify(Processor):
    ...
    def preprocess(
        ...
        map_x, map_y = forward_maps(calibration, height, width)
...
def demonstration(image: np.ndarray, calibration: Calibration) -> np.ndarray:
    """Three panels: input, the fisheye zone the view keeps (rest black), and the rectified view."""
    height, width = image.shape[:2]
    inverse_x, inverse_y = inverse_maps(calibration, height, width, height, width)
    covered = (inverse_x >= 0) & (inverse_x < width) & (inverse_y >= 0) & (inverse_y < height)
    zone = np.zeros_like(image)
    zone[covered] = image[covered]
    forward_x, forward_y = forward_maps(calibration, height, width)
